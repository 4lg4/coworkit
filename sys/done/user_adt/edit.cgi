...
<head>
  ...
  <script language='JavaScript'>
	...
	function errojx(XMLHttpRequest, textStatus, errorThrown)
        ...
        top.\$.DDialog({
            type : "alert",
            message : "ERRO!<br><br>"+XMLHttpRequest+" "+textStatus+" "+errorThrown
        });
	...
	function excluir()
        ...
        top.\$.DDialog({
            type : "confirm",
            message : 'Você tem certeza que deseja excluir esse registro?<br>Essa ação é IRREVERSÍVEL!',
            btnYes : function(){
                main.excluir_confirm();
            }
        });
	...
	function cancelar() {
        top.\$.DDialog({
            type : "confirm",
            message : 'Você tem certeza que deseja cancelar?<br>Todos os dados modificados serão perdidos!',
            btnYes : function(){
                top.main.ver();
            }
        });
		...
		if(tmp_items.length > 0)
			{
                top.\$.DDialog({
                    type : "confirm",
                    message : 'Alguns atributos já tem dados cadastros...<br><br>Confirma a exclusão desses dados?',
                    btnYes : function(){
                        // top.main.salvar_reconfirm();
                        top.\$.DDialog({
                            type : "confirm",
                            message : 'Você tem certeza disso?<br><br>Haverá perda de dados irreversível!!!',
                            btnYes : function(){
                                top.main.salvar_del();
                            }
                        });
	...
	function START()
       ...
       //  top.eos.menu.action.show(["icon_user_adt_new"]);
...
print<<HTML;
	...
	<div style='float: left; width: 47%; position: absolute; top: 2%; left: 30px; background: #3167B3; padding-bottom: 0.5%;'>
		...
		<div class="navigateable_box" style="height: 90%; overflow-x: hidden; overflow-y: hidden; background-color: white; -moz-border-radius: 0 0px 5px 5px; -webkit-border-top-right-radius: 0px; border-top-right-radius: 0px;">
...
print<<HTML;
	...
	<div style='float: left; width: 47%; position: absolute; top: 35px; left: 30px; background: #3167B3; padding-bottom: 0.5%;'>
		<div class='fake_aba' style='width: 40%; margin-left: 0px; padding: 5px;'>Atributos selecionados</div>
...
$SQL = "select * from user_item join tipo_grupo_item on user_item.tipo = tipo_grupo_item.codigo ";
$sth9 = &select("select * from information_schema.columns where table_name = 'user_item' and column_name = 'parceiro' ");
$rv9 = $sth9->rows();
if($rv9 > 0)
	{
	$SQL .= " join parceiro_tipo_grupo_item on tipo_grupo_item.codigo = parceiro_tipo_grupo_item.tipo_grupo_item and parceiro_tipo_grupo_item.parceiro = user_item.parceiro and parceiro_tipo_grupo_item.parceiro = '$LOGEMPRESA' ";
	}
$SQL .= "order by seq";
$sth = &select($SQL);
$rv = $sth->rows();
if($rv > 0)
	...
	while($row = $sth->fetchrow_hashref)
		...
		$SQL4 = "select * from empresa_user_adicional where user_item = '".$row->{'tipo'}."' ";
		$sth4 = &select($SQL4);
		$rv4 = $sth4->rows();
		if($rv4 > 0)
			{
