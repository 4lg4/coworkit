...
# $nacess = "405' and usuario_direitos.tipo = 'a";
$nacess = "27";
require "../../cfg/init.pl";
$ID = &get('ID');
$FORCE = &get('FORCE');
...
for($f=0; $f<@item; $f++)
	...
	if($item[$f] ne "")
		...
		$sth = $dbh->prepare("select * from user_item where tipo = '$item[$f]' ");
		$sth->execute;
		if($dbh->err ne "")
			{
			...
			if($rv == 0)
				...
				if($SQL2 ne "")
					{
					$rv = $dbh->do("insert into user_item (parceiro, tipo, seq) values ('$LOGEMPRESA', '$item[$f]', '$c') ");
					}
				else
					{
					$rv = $dbh->do("insert into user_item (tipo, seq) values ('$item[$f]', '$c') ");
					}
				if($dbh->err ne "")
					{
				...
				{
				$rv = $dbh->do("update user_item set seq='$c' where tipo = '$item[$f]' $SQL2 ");
				if($dbh->err ne "")
					{
					&erroDBH("Falha na alteração dos itens do grupo!!!");
					$dbh->rollback;
...
if($SQL ne "")
	...
	if($FORCE eq "S")
		{
		$sth2 = $dbh->prepare("select * from user_item where $SQL $SQL2 ");
		$sth2->execute;
		if($dbh->err ne "")
			{
			...
			if($rv2 > 0)
				...
				while($row2 = $sth2->fetchrow_hashref)
					...
					if($row2->{'tipo'} ne "")
						{
						$rv = $dbh->do("delete from empresa_user_adicional where user_item = '".$row2->{'tipo'}."' ");
						if($dbh->err ne "")
							{
	...
	$rv = $dbh->do("delete from user_item where $SQL $SQL2 ");
	if($dbh->err ne "")
		{
		&erroDBH("Falha na exclusão dos itens do grupo!!!");
		$dbh->rollback;
...
<script language='JavaScript'>
	function START()
		{
		top.call('/sys/done/user_adt/edit.cgi');
		}
	START();
</script>
</body></html>
