...
if($action eq "list")
	...
	if($COD ne "")
		{
		$DB = &DBE(" select * from default_avatar where menu='$COD' and arquivo is not null ");
		}
	#Se for para pegar todos
	else
		{
		$DB = &DBE(" select * from default_avatar where arquivo is not null ");
		}
...
#delete file
	...
	$DB1 = &DBE("delete from default_avatar where arquivo='$arquivo' ");
...
#Insere na tabela
	...
	{
	#Se for dafault upload
	$DB1 = &DBE("insert into default_avatar (menu,arquivo) values('$COD','$arquivo'); ");
